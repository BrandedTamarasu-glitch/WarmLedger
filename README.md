...
Warm Ledger supports resident data schema versions 3 through 6. All four are normalized to the same decimal-facing runtime money values while retaining their resident persistence contract. Existing unversioned, schema-version-1, and schema-version-2 data is migrated deterministically in memory to version 3 when opened; loading alone does not rewrite browser storage. Ordinary successful edits preserve the resident version. The separately confirmed exact-money upgrade moves an eligible version-3 ledger to version 4, and the first confirmed manual-clearing change to a version-4 ledger moves it atomically to version 5 after a required safety snapshot. Schema version 6 adds an optional ordered catalog of explicit local account records plus optional account references on saved records and recurring templates. Archived accounts remain valid on historical references but cannot be newly assigned, and an empty catalog or `No account selected` remains valid. These are planning labels only: they do not connect to a bank, prove payment, track balances, or reconcile activity. Moving an eligible schema-version-5 ledger to version 6 is an explicit, preview-first operation protected by a verified safety snapshot. Failed writes leave the original stored bytes and last committed in-memory budget unchanged.
