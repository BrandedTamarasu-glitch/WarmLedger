...
- A **Structure** view for adding, renaming, archiving, restoring, and reordering categories, preset expense items, earners, and schema-version-6-or-later local accounts.
...
- A write-free **Data Health** view for incomplete or unusual ledger records, compare-only backup analysis, explicit preview-first month-sharded persistence migration for eligible ledgers, and published schema-version-5-to-6 local-account plus schema-version-6-to-7 actual-account-label migration flows.
...
Warm Ledger supports resident data schema versions 3 through 7. All five are normalized to the same decimal-facing runtime money values while retaining their resident persistence contract. Existing unversioned, schema-version-1, and schema-version-2 data is migrated deterministically in memory to version 3 when opened; loading alone does not rewrite browser storage. Ordinary successful edits preserve the resident version. The separately confirmed exact-money upgrade moves an eligible version-3 ledger to version 4, and the first confirmed manual-clearing change to a version-4 ledger moves it atomically to version 5 after a required safety snapshot. Eligible schema-version-5 ledgers can then use Data Health for an explicit, preview-first migration to version 6. Schema version 6 adds an optional ordered catalog of explicit local account records plus optional planned-account references on saved records and recurring templates. Schema version 7 adds an independent optional `actualAccountId` field to saved paychecks and expenses only; values must be explicitly entered against a record with an entered actual amount and saved date, and are never inferred from planned accounts, payment methods, funding, or clearing. Store mutations validate account compatibility, clear the label when the actual amount or saved date is removed, and reset it for copied or recurring-generated records. The read-only selected-month summary counts and groups only eligible records with explicitly entered actual-account labels, in Structure order. Archived accounts remain valid on unchanged historical references but cannot be newly assigned. These labels do not connect to a bank, prove payment or transfer, track balances, or reconcile activity. Moving an eligible schema-version-6 ledger to version 7 is an explicit preview-first operation protected by a verified safety snapshot. Failed writes leave the original stored bytes and last committed in-memory budget unchanged.

Backup and snapshot envelopes remain format version 1 and can contain resident schema-version-3 through schema-version-7 data, as well as older ZeroBudget data that Warm Ledger migrates during validation. Current backups preserve the active resident version. Newer resident data may be rejected by older application versions, so download and retain a backup before intentionally downgrading.
